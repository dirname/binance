...
// NewUSDFuturesCompositeIndexWebsocketClient Factory function
func NewUSDFuturesCompositeIndexWebsocketClient(streams ...string) *FuturesCompositeIndexWebsocketClient {
	c := new(FuturesCompositeIndexWebsocketClient)
...
func (u *FuturesCompositeIndexWebsocketClient) handleMessage(msg []byte) (interface{}, error) {
	var parser map[string]interface{}
	var err error
	err = json.Unmarshal(msg, &parser)
	if _, ok := parser["stream"]; ok {
		result := CompositeIndexCombinedResponse{}
