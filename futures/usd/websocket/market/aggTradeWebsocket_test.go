...
func TestNewUSDTFuturesAggTradeWebsocketClient(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := NewUSDFuturesAggTradeWebsocketClient(tt.args.streams...); reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewUSDFuturesAggTradeWebsocketClient() = %v, want %v", got, tt.want)
			}
		})
	}
}
