...
func (u *AccountWebsocketClient) handleMessage(msg []byte) (interface{}, error) {
	...
	if _, ok := parser["stream"]; ok {
		...
		if judge.Data.EventType == "outboundAccountPosition" {
			result := AccountCombinedPosition{}
