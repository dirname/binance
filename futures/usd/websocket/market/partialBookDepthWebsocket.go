...
// NewUSDFuturesPartialBookDepthWebsocketClient Factory function
func NewUSDFuturesPartialBookDepthWebsocketClient(streams ...string) *FuturesPartialBookDepthWebsocketClient {
	c := new(FuturesPartialBookDepthWebsocketClient)
...
func (u *FuturesPartialBookDepthWebsocketClient) handleMessage(msg []byte) (interface{}, error) {
	var parser map[string]interface{}
	var err error
	err = json.Unmarshal(msg, &parser)
	if _, ok := parser["stream"]; ok {
		result := PartialBookDepthCombinedResponse{}
