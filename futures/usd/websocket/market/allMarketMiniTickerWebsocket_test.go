...
func TestFuturesAllMarketMiniTickerWebsocketClient_handleMessage(t *testing.T) {
	...
	tests := []struct {
		...
		{"TestFuturesAllMarketMiniTickerWebsocketClient_handleMessage", fields{WebsocketClient: binance.WebsocketClient{}}, args{msg: []byte("{\"stream\":\"test\"}")}, AllMarketMiniTickerCombinedResponse{
			StreamName: "test",
		}, false},
		{"TestFuturesAllMarketMiniTickerWebsocketClient_handleMessage", fields{WebsocketClient: binance.WebsocketClient{}}, args{msg: []byte("[{\"e\":\"test\"}]")}, AllMarketMiniTickerResponse{{
			EventType: "test",
		}}, false},
		{"TestFuturesAllMarketMiniTickerWebsocketClient_handleMessage", fields{WebsocketClient: binance.WebsocketClient{}}, args{msg: []byte("{\"result\":\"test\"}")}, model.WebsocketCommonResponse{
			Result: "test",
		}, false},
		// TODO: Add test cases.
	}
...
func TestNewUSDFuturesAllMarketMiniTickerWebsocketClient(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := NewUSDFuturesAllMarketMiniTickerWebsocketClient(tt.args.streams...); reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewUSDFuturesAllMarketMiniTickerWebsocketClient() = %v, want %v", got, tt.want)
			}
		})
	}
}
