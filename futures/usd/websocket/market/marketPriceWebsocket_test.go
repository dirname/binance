...
func TestNewUSDTFuturesMarketPriceWebsocketClient(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := NewUSDFuturesMarketPriceWebsocketClient(tt.args.streams...); reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewUSDFuturesMarketPriceWebsocketClient() = %v, want %v", got, tt.want)
			}
		})
	}
}
