...
// NewUSDFuturesAllMarketMiniTickerWebsocketClient Factory function
func NewUSDFuturesAllMarketMiniTickerWebsocketClient(streams ...string) *FuturesAllMarketMiniTickerWebsocketClient {
	c := new(FuturesAllMarketMiniTickerWebsocketClient)
...
func (u *FuturesAllMarketMiniTickerWebsocketClient) handleMessage(msg []byte) (interface{}, error) {
	var parser map[string]interface{}
	var err error
	err = json.Unmarshal(msg, &parser)
	if msg[0] == 91 {
		var result AllMarketMiniTickerResponse
		err = json.Unmarshal(msg, &result)
		return result, err
	}
	if _, ok := parser["stream"]; ok {
		result := AllMarketMiniTickerCombinedResponse{}
		err = json.Unmarshal(msg, &result)
		return result, err
	}
