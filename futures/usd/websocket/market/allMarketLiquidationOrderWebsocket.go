...
// NewUSDFuturesAllMarketLiquidationOrderWebsocketClient Factory function
func NewUSDFuturesAllMarketLiquidationOrderWebsocketClient(streams ...string) *FuturesAllMarketLiquidationOrderWebsocketClient {
	c := new(FuturesAllMarketLiquidationOrderWebsocketClient)
...
func (u *FuturesAllMarketLiquidationOrderWebsocketClient) handleMessage(msg []byte) (interface{}, error) {
	var parser map[string]interface{}
	var err error
	err = json.Unmarshal(msg, &parser)
	if _, ok := parser["stream"]; ok {
		result := AllMarketLiquidationOrderCombinedResponse{}
	...
	if _, ok := parser["e"]; ok {
		result := AllMarketLiquidationOrderResponse{}
		err = json.Unmarshal(msg, &result)
		return result, err
	}
