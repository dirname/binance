...
// NewUSDFuturesBLVTInfoClient Factory function
// stream's token name must be uppercase, e.g. "TRXDOWN@tokenNav"
func NewUSDFuturesBLVTInfoClient(streams ...string) *FuturesBLVTInfoWebsocketClient {
	c := new(FuturesBLVTInfoWebsocketClient)
...
func (u *FuturesBLVTInfoWebsocketClient) handleMessage(msg []byte) (interface{}, error) {
	var parser map[string]interface{}
	var err error
	err = json.Unmarshal(msg, &parser)
	if _, ok := parser["stream"]; ok {
		result := BLVTInfoCombinedResponse{}
