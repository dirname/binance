...
func TestNewUSDFuturesPartialBookDepthWebsocketClient(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := NewUSDFuturesPartialBookDepthWebsocketClient(tt.args.streams...); reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewUSDFuturesPartialBookDepthWebsocketClient() = %v, want %v", got, tt.want)
			}
		})
	}
}
