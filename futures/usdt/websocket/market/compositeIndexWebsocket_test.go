...
func TestNewUSDTFuturesCompositeIndexWebsocketClient(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := NewUSDTFuturesCompositeIndexWebsocketClient(tt.args.streams...); reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewUSDTFuturesCompositeIndexWebsocketClient() = %v, want %v", got, tt.want)
			}
		})
	}
}
