...
func TestNewOrder(t1 *testing.T) {
	...
	tests := []struct {
		...
		{"TestNewOrder", fields{binance.NewPrivateUrlBuilder("", "", "")}, args{
			symbol:           "test",
			side:             "test",
			orderType:        order.TakeProfitLimit,
			timeInForce:      "test",
			newClientOderID:  "test",
			newOrderRespType: "test",
			quantity:         decimal.NewFromInt(100),
