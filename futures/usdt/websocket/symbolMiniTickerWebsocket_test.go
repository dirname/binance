...
func TestNewUSDTFuturesSymbolMiniTickerWebsocketClient(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := NewUSDTFuturesSymbolMiniTickerWebsocketClient(tt.args.streams...); reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewUSDTFuturesSymbolMiniTickerWebsocketClient() = %v, want %v", got, tt.want)
			}
		})
	}
}
